...
import {
  ...
  ExternalLink, Search, Archive, Globe, Box,
} from "lucide-react";
...
type ProjectTab = "brief" | "hologram";

const DEMO_PROJECT_IDS = ["rgm-machine", "otter-cpu", "metal-detector", "funck", "air-motor"];

function HologramLoader() {
  return <div className="h-96 flex items-center justify-center text-muted-foreground font-mono text-sm animate-pulse">Loading 3D viewer...</div>;
}
...
export default function ProjectsSection() {
  ...
  const filtered = activeProjects.filter((p) => {
    ...
    const matchesSearch = !search || p.name.toLowerCase().includes(search.toLowerCase()) || p.codename.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <section className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        <div className="flex gap-1">
          {(["All", "Hardware", "Systems", "Ops", "Web"] as FilterCategory[]).map((cat) => (
            <button key={cat} onClick={() => setFilter(cat)} className={`px-3 py-1 text-xs font-mono rounded border transition-colors ${filter === cat ? "border-primary/40 bg-primary/10 text-primary" : "border-panel-border text-muted-foreground hover:text-foreground"}`}>
              {cat}
            </button>
          ))}
        </div>
        <div className="relative flex-1 max-w-xs">
          ...
          <input type="text" placeholder="Search projects..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-8 pr-3 py-1.5 text-xs font-mono bg-panel border border-panel-border rounded text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/40" />
        </div>
      </div>
        ...
        <div className="lg:col-span-3 panel-glass rounded-lg overflow-hidden">
          ...
          <div className="p-2 space-y-0.5 max-h-[70vh] overflow-y-auto">
            {filtered.map((p) => (
              <button
                key={p.id}
                onClick={() => { setSelectedProject(p); setActiveTab("brief"); }}
                className={`w-full flex items-center gap-2 px-3 py-2.5 rounded text-left transition-colors ${selectedProject.id === p.id ? "bg-primary/10 border border-primary/20" : "hover:bg-panel-highlight border border-transparent"}`}
              >
            ...
            {activeTab === "hologram" && selectedProject.has3D ? (
              <motion.div key={selectedProject.id + "-holo"} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <Suspense fallback={<HologramLoader />}>
                  {selectedProject.id === "otter-cpu" && <OtterInteractive />}
                  {selectedProject.id === "funck" && <FunckNetworkHologram />}
                  {selectedProject.id === "metal-detector" && <MetalDetectorHologram />}
                  {selectedProject.id === "air-motor" && <AirMotorHologram />}
              ...
              <motion.div key={selectedProject.id + "-brief"} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                  {/* Brief panel */}
                  <div className="lg:col-span-3 panel-glass rounded-lg overflow-hidden">
                    <PanelHeader>Project Brief — {selectedProject.codename}</PanelHeader>
                    <div className="p-4 space-y-5 max-h-[70vh] overflow-y-auto">
                      ...
                      {mode === "engineer" && (
                        <>
                          <div>
                            <h4 className="text-xs font-mono font-semibold text-primary tracking-wider mb-1.5 uppercase">Mission Objective</h4>
                            <p className="text-sm text-secondary-foreground leading-relaxed">{selectedProject.module.missionObjective}</p>
                          </div>
                          <div>
                            <h4 className="text-xs font-mono font-semibold text-primary tracking-wider mb-1.5 uppercase">System Architecture</h4>
                            <p className="text-sm text-secondary-foreground leading-relaxed">{selectedProject.module.systemArchitecture}</p>
                          </div>
                          {selectedProject.module.implementationNotes.length > 0 && (
                            <div>
                              <h4 className="text-xs font-mono font-semibold text-primary tracking-wider mb-1.5 uppercase">Implementation Notes</h4>
                              <ul className="space-y-1">
                                {selectedProject.module.implementationNotes.map((note, i) => (
                                  <li key={i} className="flex items-start gap-2 text-xs text-secondary-foreground">
                                    <span className="text-primary/50 mt-0.5 font-mono">{String(i + 1).padStart(2, "0")}</span>{note}
                                  </li>
                                ))}
                              </ul>
                            </div>
                      ...
                      {selectedProject.module.failureModes.length > 0 && (
                        <div>
                          <h4 className="text-xs font-mono font-semibold text-primary tracking-wider mb-1.5 uppercase">Failure Modes & Fixes</h4>
                      ...
                      {selectedProject.module.ownershipDisclosure && (
                        <div>
                          <h4 className="text-xs font-mono font-semibold text-primary tracking-wider mb-1.5 uppercase">What I Owned vs. AI-Assisted</h4>
                      ...
                      {selectedProject.module.improvements.length > 0 && (
                        <div>
                          <h4 className="text-xs font-mono font-semibold text-primary tracking-wider mb-1.5 uppercase">Next Steps</h4>
                          <ul className="space-y-1">{selectedProject.module.improvements.map((imp, i) => (
                            <li key={i} className="flex items-start gap-2 text-xs text-secondary-foreground"><span className="text-neon-cyan">→</span>{imp}</li>
                          ))}</ul>
                        </div>
                      )}
      ...
      {!demoMode && archivedProjects.length > 0 && (
        <div className="mt-6">
          <button onClick={() => setArchiveOpen(!archiveOpen)} className="flex items-center gap-2 text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
            <Archive size={14} />
            <span className="uppercase tracking-wider">Archive ({archivedProjects.length})</span>
                ...
                <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                  {archivedProjects.map((p) => (
                    <div key={p.id} className="panel-glass rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2"><StatusLight color={p.statusColor} /><span className="text-sm font-semibold text-foreground">{p.name}</span></div>
                      <p className="text-xs text-secondary-foreground mb-3">{p.heroSummary}</p>
                      <EvidencePending items={["Upload project documentation", "Add demonstration photos", "Document implementation details"]} />
                    </div>
                  ))}
                </div>
              </motion.div>
  ...
  return (
    <div>
      ...
      {project.evidence.length === 0 ? (
        <EvidencePending items={["Upload project documentation", "Add photos or screenshots", "Include test results"]} />
      ) : (
        <div className="space-y-2">
          {project.evidence.map((ev) => (
            <div key={ev.id} className="border border-panel-border rounded p-2.5 text-xs">
              <div className="flex items-center gap-2 mb-1">
                {ev.type === "pdf" ? <FileText size={12} className="text-neon-cyan" /> : ev.type === "link" ? <ExternalLink size={12} className="text-neon-cyan" /> : <ImageIcon size={12} className="text-neon-cyan" />}
