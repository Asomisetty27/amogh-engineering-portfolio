...
const TABS = [
  { value: "evidence",    label: "Evidence",    sub: "Claim → proof", Component: EvidenceBoard },
  { value: "milestones",  label: "Milestones",  sub: "Trajectory → W27", Component: Predictions },
  { value: "outreach",    label: "Outreach",    sub: "Funnel & quotes", Component: Outreach },
] as const;
...
export default function YC() {
  ...
  const tab: TabValue = (TABS.find((t) => t.value === raw)?.value ?? "evidence");

  const onTabChange = (v: string) => {
    const next = new URLSearchParams(params);
    if (v === "evidence") next.delete("tab"); else next.set("tab", v);
    ...
    <div>
      <div className="mb-5">
        ...
        <p className="text-[12px] text-[#888780] mt-1 max-w-2xl">
          The path to YC W27: every claim must have proof, every milestone has a trajectory, every conversation
          counts toward the founding story.
        </p>
      </div>
